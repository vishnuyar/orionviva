...
function Harness({
  ...
  return <><button onClick={() => setOpen(true)}>Open evidence</button><button onClick={dialog.cancelPendingRestore}>Cancel restore</button><button onClick={dialog.dismissAndRestore}>Schedule restore</button><button onClick={dialog.closeWithoutRestore}>Close without restore</button><h1 ref={title} tabIndex={-1}>Page</h1>{open && <aside ref={drawer} role="dialog" aria-label="Evidence" tabIndex={-1}><span style={hiddenStyle(closeAncestorCss)}><button ref={close} style={hiddenStyle(closeCss)} aria-hidden={closeState === "aria-hidden" ? "true" : undefined} disabled={closeState === "disabled"} hidden={closeState === "hidden"} onClick={dialog.dismissAndRestore}>Close evidence</button></span><button style={hiddenStyle(lastCss)} hidden={allActionsIneligible}>Last action</button><button disabled>Disabled</button></aside>}</>;
}
