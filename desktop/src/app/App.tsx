...
export function App() {
  ...
  useEffect(() => {
    if (!vaultDirectory && control.rememberedVaultDirectory) {
      setVaultDirectory(control.rememberedVaultDirectory);
    }
  }, [control.rememberedVaultDirectory, vaultDirectory]);
  useEffect(() => {
    if (control.automaticOpenTimedOut) passphraseRef.current?.focus();
  }, [control.automaticOpenTimedOut]);
  ...
  async function leaveVault() { if (openingVault) return; if (control.hostCloseAvailable && !(await control.closeVault())) return; evidenceDialog.cancelPendingRestore(); conversationDialog.cancelPendingRestore(); setConversationPlanDraft(null); setPlanActionReceipt(null); control.resetDemo(); setOverlay(null); requestAnimationFrame(() => pageTitleRef.current?.focus()); }
  ...
  return <div className={frame ? "app-shell app-shell-sample" : "app-shell"} inert={accountTransactionOpen ? true : undefined} aria-hidden={accountTransactionOpen ? true : undefined}>
    ...
    {control.closingVault && <div className="action-explanation" id="vault-close-waiting" role="status">Closing vault… Wait for OrionViva to end access.</div>}
    {frame ? <div className="sample-frame" role="note" aria-label={frame.title}><div className="sample-frame-copy"><strong>{frame.title}</strong><span>{frame.detail}</span></div><button className="secondary-button sample-frame-leave" type="button" aria-disabled={openingVault || control.closingVault} aria-describedby={control.closingVault ? "vault-close-waiting" : openingVault ? "vault-application-waiting" : undefined} onClick={leaveVault}>{control.closingVault ? "Closing vault…" : frame.leave}</button></div> : null}
    ...
    <aside ref={navigationDrawerRef} id="primary-navigation-drawer" className={mobileNav ? "sidebar sidebar-open" : "sidebar"} role={isNarrow && mobileNav ? "dialog" : undefined} aria-modal={isNarrow && mobileNav ? true : undefined} aria-labelledby={isNarrow && mobileNav ? "primary-navigation-title" : undefined} aria-hidden={isNarrow ? !mobileNav : accountTransactionOpen ? true : undefined} inert={openingVault || Boolean(evidenceSelection) || conversationOpen || accountTransactionOpen || (isNarrow && !mobileNav) ? true : undefined} tabIndex={-1}>
      ...
      {session.source ? <div className="vault-session-actions"><span>Vault open</span><button className="text-button" type="button" aria-disabled={control.closingVault} aria-describedby={control.closingVault ? "vault-close-waiting" : undefined} onClick={leaveVault}>{control.closingVault ? "Closing vault…" : session.source.frame ? session.source.frame.leave : "Close this vault"}</button></div> : null}
      ...
      <div className="vault-source-card">
        ...
        {control.hostAvailable ? <form className="vault-open-form" onSubmit={openVault}><fieldset className="vault-start-choice"><legend>Start with a private vault</legend><label className="vault-create-choice"><input type="radio" name="vault-mode" checked={!makeVault} onChange={() => setMakeVault(false)} />Open an existing vault</label><label className="vault-create-choice"><input type="radio" name="vault-mode" checked={makeVault} onChange={() => setMakeVault(true)} />Make a new vault in that folder</label></fieldset><p>{makeVault ? "Choose a folder for your new vault and a passphrase you can keep safely. The vault holds encrypted records on this computer." : "Choose the folder containing your vault, then enter its passphrase."}</p><label>Vault directory<span className="vault-directory-control"><input value={vaultDirectory} onChange={(event) => setVaultDirectory(event.target.value)} placeholder="/path/to/vault" autoComplete="off" />{control.pickerAvailable && <button className="vault-picker-button" type="button" onClick={pickVaultDirectory} aria-disabled={openingVault} aria-describedby={openingVault ? "vault-open-waiting" : undefined}><FolderOpen size={14} />{pickingVaultDirectory ? "Choosing..." : "Choose folder"}</button>}</span></label><label>Passphrase<input ref={passphraseRef} type="password" value={passphrase} onChange={(event) => setPassphrase(event.target.value)} placeholder="Enter passphrase" autoComplete={makeVault ? "new-password" : "current-password"} aria-describedby="vault-passphrase-consequence" /></label><p className="vault-passphrase-consequence" id="vault-passphrase-consequence">Keep your passphrase somewhere safe: it is needed to unlock a copy on another device. Opening an existing vault never creates one. {control.remembersVault ? "After a successful open, this device protects the vaultphrase in macOS Keychain or Windows Credential Manager and opens this vault by default. Choosing another vault replaces that default." : "Keep your vaultphrase available. This session does not save it for automatic opening after restarting OrionViva."} The vault itself never stores the vaultphrase, and moving it to another device still requires the vaultphrase there.</p><button className="secondary-button vault-open-button" type="submit" aria-disabled={openingVault} aria-describedby={openingVault ? "vault-passphrase-consequence vault-open-waiting" : "vault-passphrase-consequence"}>{openingVault ? "Opening vault..." : makeVault ? "Make and open vault" : "Open local vault"}</button>{openingVault ? <span className="action-explanation" id="vault-open-waiting">Your vault is answering the last request. Pressing again does nothing until it has.</span> : null}</form> : <span className="vault-host-note">Preview mode. A desktop host bridge will enable local vault opening.</span>}
      </div>
      </details>
