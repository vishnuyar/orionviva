  ...
  return <PanelStateView result={result} copy={{ partial: "Some financial-picture details are unavailable. Available details are shown below.", needsInput: "Some parts of the financial picture need more information. Available details are shown below.", unavailable: { title: "Financial picture unavailable", detail: "The financial picture is not available in this build." }, failed: { title: "Financial picture could not be read", detail: "The financial picture could not be read. The private vault is still open." } }}>{(data) => {
    ...
    return <>
      <section className="hero-grid">
        <div className="hero-card">{data.netWorth ? <>
          <div className="card-topline"><span>Net worth</span><EvidenceBadge grade={data.netWorth.grade} label={data.netWorth.gradeLabel} description={data.netWorth.gradeDescription} /></div>
          <Figure figure={netWorthEvidenceFigure(data.netWorth)} onOpenEvidence={onOpenFigure} className="hero-amount" />
          <div className="hero-meta"><span>{data.netWorth.gradeLabel}{data.netWorth.asOf && ` · ${data.netWorth.asOf}`}</span></div>
          <div className="figure-footnote">{data.netWorth.coverage && <span>{data.netWorth.coverage}</span>}{data.netWorth.caveats[0] && <small>{data.netWorth.caveats[0]}</small>}</div>
          <ProofLinks label="View source" links={data.netWorth.evidenceLinks} onOpen={onOpenEvidence} />
        </> : <div className="empty-state"><strong>{mode === "demo" ? "No sample net-worth figure" : "Net worth isn’t available in this preview"}</strong><span>{mode === "demo" ? "This fictional sample does not include a net-worth figure." : "The current vault read does not provide a net-worth figure. No total has been calculated in the interface."}</span></div>}</div>
        <div className="coverage-card"><div className="card-topline"><span>Picture coverage</span><CircleHelp size={16} /></div>
          <dl className="overview-read-context"><div><dt>Current-through value</dt><dd>{data.currentThrough.trim() ? data.currentThrough : mode === "demo" ? "Sample current-through value was not authored." : "Current-through value was not supplied by this overview read."}</dd></div></dl><p>Shown as supplied. The interface does not infer freshness.</p>
          {mode === "demo" ? <><div className="coverage-number">{data.coverage.trim() ? data.coverage : "Sample coverage was not authored."}</div><p>{data.corpusCoverage.trim() ? `${data.corpusCoverage}. The sample stays visibly incomplete.` : "Sample corpus coverage was not authored. The sample does not claim completeness."}</p></> : <p>Coverage totals were not supplied by this overview read. Available accounts are shown individually.</p>}
      ...
      <section className="section-block"><div className="section-heading"><div><div className="section-kicker">Current review read</div><h2>Review queue</h2></div><button type="button" className="text-button" onClick={() => onNavigate("review")}>Open review <ArrowUpRight size={14} /></button></div><div className="queue-list">{identifiedRows(reviewData?.queue ?? [], "overview-review").map((row) => {
        ...
        return <div className="queue-row" key={row.key}><div className="queue-copy"><div className="queue-title"><strong>{reviewLabel(row.item, mode)}</strong><span>{mode === "demo" ? "Fictional sample" : "Read only"}</span></div></div><button type="button" className="secondary-button" onClick={() => onOpenReviewQuestion(row.item.id)}>View question</button></div>;
      })}</div></section>
