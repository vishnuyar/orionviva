...
function AccountDetail({ selection, onOpenEvidence, onOpenFigure }: { selection: StableSelection<AccountView>; onOpenEvidence: (link: EvidenceLink) => void; onOpenFigure: (figureId: string) => void }) {
  if (selection.state === "ready") {
    const account = selection.item;
    const figure = accountEvidenceFigure(account);
    return <aside className="detail-panel" aria-labelledby="selected-account-title"><div className="detail-panel-label">Selected account</div><h3 id="selected-account-title" tabIndex={-1}>{accountName(account)}</h3><Figure figure={figure} onOpenEvidence={onOpenFigure} className="selected-account-figure" />{account.coverage && <p>{account.coverage}</p>}<div className="detail-panel-grid"><div><span>Kind</span><strong>{accountKind(account)}</strong></div><div><span>Grade</span><strong>{figure.grade.label}: {figure.grade.description}</strong></div><div><span>Measured</span><strong>{account.asOf || "Not supplied"}</strong></div><div><span>Note</span><strong>{account.note || "Not supplied"}</strong></div></div><ProofLinks label="Supporting documents" links={account.evidenceLinks} onOpen={onOpenEvidence} /></aside>;
  }
...
export function Accounts({ result, selectedAccount, onSelectAccount, onOpenEvidence, onOpenFigure, onExploreSample }: { result: FeatureResult<OverviewData>; selectedAccount: string; onSelectAccount: (id: string) => void; onOpenEvidence: (link: EvidenceLink) => void; onOpenFigure: (figureId: string) => void; onExploreSample: () => void }) {
  return <PanelStateView result={result} copy={{ partial: "Some account details are unavailable. Available accounts are shown below.", needsInput: "Some accounts need more information. Available account details are shown below.", unavailable: { title: "Accounts unavailable", detail: "Account details are not available in this build." }, failed: { title: "Accounts could not be read", detail: "The accounts section could not be read. The private vault is still open." } }}>{(data) => {
    ...
    return <section className="feature-panel"><div className="feature-icon">◎</div><h2>Accounts in this read</h2><p>Each account is shown independently with the fields supplied to this view.</p>
      <div className="account-detail-list">{identifiedRows(data.accounts, "accounts-list").map((row) => {
        ...
        return <div className="detail-row" key={row.key}><button type="button" className={pressed ? "detail-row-button active" : "detail-row-button"} aria-pressed={pressed} onClick={() => onSelectAccount(account.id)}><div><strong>{accountName(account)}</strong><span>{accountKind(account)}{account.note ? ` · ${account.note}` : ""}</span></div></button><div className="detail-figure"><Figure figure={accountEvidenceFigure(account)} onOpenEvidence={onOpenFigure} />{account.asOf && <span>{account.asOf}</span>}</div></div>;
      })}</div>
      <AccountDetail selection={selection} onOpenEvidence={onOpenEvidence} onOpenFigure={onOpenFigure} />
    </section>;
  }}</PanelStateView>;
}
