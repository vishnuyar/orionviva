...
describe("Conversation body", () => {
  it("renders all six states while leaving the absent body empty", () => {
    const { container, getByText, queryByText, rerender } = render(<ConversationDrawer ask={null} mode="live" result={{ state: "absent", reason: "not_read" }} selectedPrompt="" onSelectPrompt={noAction} />);
    expect(container).toBeEmptyDOMElement();
    rerender(<ConversationDrawer ask={null} mode="live" result={{ state: "unavailable", reason: "not_connected" }} selectedPrompt="" onSelectPrompt={noAction} />);
    ...
    rerender(<ConversationDrawer ask={null} mode="live" result={{ state: "failed", reason: "read_failed" }} selectedPrompt="" onSelectPrompt={noAction} />);
    ...
    expect(queryByText(/No prompt was sent and no model call was made/)).not.toBeInTheDocument();
    rerender(<ConversationDrawer ask={null} mode="live" result={{ state: "partial", data: { turns: [turn("partial")], prompts: [] }, issues: [{ code: "partial", message: "bounded" }] }} selectedPrompt="" onSelectPrompt={noAction} />);
    expect(getByText("Some conversation details are unavailable. Supplied read-only content is shown below.")).toBeInTheDocument();
    rerender(<ConversationDrawer ask={null} mode="live" result={{ state: "needs_input", data: { turns: [turn("needs")], prompts: [] }, issues: [{ code: "needs", message: "bounded" }] }} selectedPrompt="" onSelectPrompt={noAction} />);
  ...
  it("keeps demo and live truth domains isolated, including honest empties", () => {
    ...
    const { getByText, queryByText, rerender } = render(<ConversationDrawer ask={null} mode="demo" result={ready({ ...maliciousLive, sample: { turns: [], prompts: [] } })} selectedPrompt="" onSelectPrompt={noAction} />);
    ...
    rerender(<ConversationDrawer ask={null} mode="live" result={ready({ turns: [], prompts: [], sample: { turns: [turn("sample-secret", "Sample secret")], prompts: [prompt("sample-prompt", "Sample prompt")] } })} selectedPrompt="" onSelectPrompt={noAction} />);
  ...
  it("renders supplied live fields without implying a model call", () => {
    ...
    const { container, getAllByText, getByText, queryByRole } = render(<ConversationDrawer ask={null} mode="live" result={ready(data)} selectedPrompt="" onSelectPrompt={noAction} />);
  ...
  it("labels demo speakers as fictional and preserves supplied turn order without claiming chronology", () => {
    ...
    const view = render(<ConversationDrawer ask={null} mode="live" result={ready({ turns: supplied, prompts: [prompt("prompt")] })} selectedPrompt="" onSelectPrompt={noAction} />);
    ...
    view.rerender(<ConversationDrawer ask={null} mode="demo" result={ready({ turns: [], prompts: [], sample: { turns: [{ ...turn("viva"), speaker: "viva" }, { ...turn("person"), speaker: "you" }], prompts: [prompt("prompt")] } })} selectedPrompt="" onSelectPrompt={noAction} />);
  ...
  it("selects exact prompt IDs with native pressed state and no focus movement", () => {
    ...
    const { getAllByRole, rerender } = render(<ConversationDrawer ask={null} mode="demo" result={ready({ turns: [], prompts: [], sample: { turns: [], prompts } })} selectedPrompt="" onSelectPrompt={onSelect} />);
    ...
    rerender(<ConversationDrawer ask={null} mode="demo" result={ready({ turns: [], prompts: [], sample: { turns: [], prompts: [...prompts].reverse() } })} selectedPrompt="two" onSelectPrompt={onSelect} />);
  ...
  it("bounds blank, duplicate, missing, conflicted, and no-selectable prompt identities", () => {
    ...
    const view = render(<ConversationDrawer ask={null} mode="live" result={ready({ turns: [], prompts: unusable })} selectedPrompt="" onSelectPrompt={noAction} />);
    ...
    view.rerender(<ConversationDrawer ask={null} mode="live" result={ready({ turns: [], prompts: [prompt("present")] })} selectedPrompt="missing" onSelectPrompt={noAction} />);
    expect(view.getByText("Selected prompt unavailable")).toBeInTheDocument();
    expect(view.getByText("missing")).toBeInTheDocument();
    view.rerender(<ConversationDrawer ask={null} mode="live" result={ready({ turns: [], prompts: [prompt("x"), prompt("x")] })} selectedPrompt="x" onSelectPrompt={noAction} />);
  ...
  it("groups blank and duplicate turns once and states missing text without inference", () => {
    ...
    const { getAllByText, getByText, queryByText } = render(<ConversationDrawer ask={null} mode="demo" result={ready({ turns: [], prompts: [], sample: { turns, prompts: [prompt("prompt")] } })} selectedPrompt="" onSelectPrompt={noAction} />);
  ...
  it("preserves long IDs, states missing prompt fields by mode, and keeps the list before detail", () => {
    ...
    const view = render(<ConversationDrawer ask={null} mode="demo" result={ready({ turns: [], prompts: [], sample: { turns: [], prompts: [missing] } })} selectedPrompt="" onSelectPrompt={noAction} />);
    ...
    view.rerender(<ConversationDrawer ask={null} mode="live" result={ready({ turns: [], prompts: [missing] })} selectedPrompt="" onSelectPrompt={noAction} />);
