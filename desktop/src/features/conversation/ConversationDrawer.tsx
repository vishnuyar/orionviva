...
import type { ActionOutcome, AskActionState, ConversationData, ConversationTurn, DeclineReason, FeatureResult, QuestionActionState, TurnView } from "../../surface/types";
...
function AnswerDetail({ answer, onOpenFigure }: { answer: TurnView; onOpenFigure: (figureId: string) => void }) {
...
function Timeline({ data, controls, onOpenFigure }: { data: ConversationData; controls: ConversationControls; onOpenFigure: (figureId: string) => void }) {
  ...
  return <section className="conversation-thread" aria-labelledby="conversation-turns-title">
    ...
    <ol>{data.turns.map((turn) => <li className={`conversation-turn ${turn.outcome}`} key={turn.id}>
      ...
      {turn.answer ? <AnswerDetail answer={turn.answer} onOpenFigure={onOpenFigure} /> : turn.message ? <p className="conversation-answer-text">{turn.message}</p> : null}
...
export function ConversationDrawer({ result, selectedQueue, onSelectQueue, onOpenFigure, ask, controls }: { result: FeatureResult<ConversationData>; selectedQueue: string; onSelectQueue: (id: string) => void; onOpenFigure: (figureId: string) => void; ask: AskControls | null; controls: ConversationControls }) {
  ...
  return <>
    ...
    <PanelStateView result={result} copy={{ partial: "Some conversation details are unavailable.", needsInput: "This conversation needs input.", unavailable: { title: "Conversation unavailable", detail: "This build cannot read the conversation for this vault." }, failed: { title: "Conversation could not be read", detail: "The durable conversation read did not complete." } }}>
      {(data) => {
        ...
        return <div className="conversation-body"><Timeline data={data} controls={controls} onOpenFigure={onOpenFigure} /><Questions result={{ state: "ready", data: data.questions }} selectedQueue={selectedQueue} onSelectQueue={onSelectQueue} actions={questionControls} /></div>;
      }}
    </PanelStateView>
