...
function TurnList({ turns, mode }: { turns: readonly ConversationTurn[]; mode: SurfaceMode }) {
  return <section className="conversation-thread" aria-labelledby="conversation-turns-title"><h3 id="conversation-turns-title">{mode === "demo" ? "Fictional conversation turns" : "Supplied conversation turns"}</h3><p className="conversation-turn-order">Turns are shown in the order supplied to this view. The interface does not verify chronology.</p><ul>{presentConversationRows(turns, "turn").map((row) => {
    ...
    const turn = row.item;
    return <li className={`conversation-turn ${turn.state}`} key={row.key}><div className="conversation-turn-meta"><strong>{mode === "demo" ? (turn.speaker === "viva" ? "Fictional Viva" : "Fictional person") : `Supplied speaker: ${turn.speaker}`}</strong><span>{turn.state}</span></div><p>{turn.text.trim() ? turn.text : mode === "demo" ? "Sample turn text was not authored." : "Turn text was not supplied by this conversation view."}</p>{turn.citation && <div className="conversation-citation"><span>{mode === "demo" ? "Fictional citation text:" : "Supplied citation text:"} {turn.citation}</span><small>Citation text is not a document link. This conversation view does not supply a document identity.</small></div>}<dl><div><dt>Turn ID</dt><dd>{turn.id}</dd></div></dl></li>;
  })}</ul></section>;
}

function PromptRows({ prompts, selectedPrompt, selection, mode, onSelectPrompt }: { prompts: readonly ConversationPrompt[]; selectedPrompt: string; selection: PromptSelection; mode: SurfaceMode; onSelectPrompt: (id: string) => void }) {
  return <section className="conversation-prompts" aria-labelledby="conversation-prompts-title"><div className="conversation-section-heading"><div><h3 id="conversation-prompts-title">{mode === "demo" ? "Sample prompts" : "Supplied prompts"}</h3><p>{mode === "demo" ? "Choose a fictional prompt to inspect its authored detail. Nothing is sent." : "Choose a supplied prompt to inspect its detail. Nothing is sent."}</p></div><span className="conversation-readonly-badge">{mode === "demo" ? "Fictional sample" : "Read only"}</span></div><ul className="conversation-prompt-list">{presentConversationRows(prompts, "prompt").map((row) => {
    ...
    return <li key={row.key}><button type="button" className={pressed ? "conversation-prompt active" : "conversation-prompt"} aria-pressed={pressed} onClick={() => onSelectPrompt(prompt.id)}><strong>{prompt.label.trim() ? prompt.label : mode === "demo" ? "Sample prompt label was not authored." : "Prompt label was not supplied by this conversation view."}</strong><span>{prompt.detail.trim() ? prompt.detail : mode === "demo" ? "Sample prompt detail was not authored." : "Prompt detail was not supplied by this conversation view."}</span><span className="conversation-prompt-id">Prompt ID: {prompt.id}</span><small>{prompt.state}</small></button></li>;
  })}</ul><SelectedPrompt selection={selection} mode={mode} selectedPrompt={selectedPrompt} /></section>;
}

function SelectedPrompt({ selection, mode, selectedPrompt }: { selection: PromptSelection; mode: SurfaceMode; selectedPrompt: string }) {
  if (selection.state === "ready") {
    const prompt = selection.prompt;
    return <aside className="conversation-selected" aria-labelledby="selected-conversation-prompt-title"><div className="detail-panel-label">Selected prompt</div><h3 id="selected-conversation-prompt-title">{prompt.label.trim() ? prompt.label : mode === "demo" ? "Sample prompt label was not authored." : "Prompt label was not supplied by this conversation view."}</h3><p>{prompt.detail.trim() ? prompt.detail : mode === "demo" ? "Sample prompt detail was not authored." : "Prompt detail was not supplied by this conversation view."}</p><dl><div><dt>Prompt ID</dt><dd>{prompt.id}</dd></div><div><dt>Prompt state</dt><dd>{prompt.state}</dd></div></dl></aside>;
  }
...
function ConversationReady({ data, mode, selectedPrompt, onSelectPrompt }: { data: ConversationData; mode: SurfaceMode; selectedPrompt: string; onSelectPrompt: (id: string) => void }) {
  const view = mode === "demo" ? data.sample ?? { turns: [], prompts: [] } : { turns: data.turns, prompts: data.prompts };
  if (!view.turns.length && !view.prompts.length) return <div className="empty-state"><strong>{mode === "demo" ? "No sample conversation" : "No conversation content supplied"}</strong><span>{mode === "demo" ? "This fictional sample does not include any turns or prompts. No prompt was sent and no model call was made." : "This supplied Conversation view contains no turns or prompts. Empty content does not establish whether a model call occurred."}</span></div>;
