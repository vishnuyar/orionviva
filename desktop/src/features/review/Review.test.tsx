...
describe("Review inspection surface", () => {
  it("renders live fields exactly and never combines supplied amount and currency", () => {
    ...
    const { container, getByRole, getByText, queryByText, queryByRole } = render(<Review result={ready(data([live]))} selectedQueue="live-question" onSelectQueue={noAction} actions={inert} />);
  ...
  it("renders every missing live field explicitly", () => {
    ...
    const { getAllByText, getByText } = render(<Review result={ready(data([missing]))} selectedQueue="missing-fields" onSelectQueue={noAction} actions={inert} />);
  ...
  it("keeps blank, duplicate, missing, and no-selectable identities bounded", () => {
    ...
    const { container, getAllByText, getByRole, getByText, queryByRole, rerender } = render(<Review result={ready(data(duplicate))} selectedQueue="duplicate" onSelectQueue={noAction} actions={inert} />);
    ...
    rerender(<Review result={ready(data([question("present")]))} selectedQueue="missing-question-id" onSelectQueue={noAction} actions={inert} />);
    ...
    rerender(<Review result={ready(data([question(""), question(" ")]))} selectedQueue="" onSelectQueue={noAction} actions={inert} />);
    expect(getByRole("heading", { name: "Question selection unavailable" })).toHaveAttribute("tabindex", "-1");
    expect(container.querySelectorAll("#selected-question-title")).toHaveLength(1);
  ...
  it("keeps ordinary list selection on the pressed button", () => {
    ...
    const { getByRole } = render(<Review result={ready(data(queue))} selectedQueue="first" onSelectQueue={select} actions={inert} />);
  ...
  it("renders pending, tail, and guidance independently without association", () => {
    ...
    const { getByText, queryByRole, queryByText, rerender } = render(<Review result={ready(review)} selectedQueue="" onSelectQueue={noAction} actions={inert} />);
    ...
    rerender(<Review result={ready(data([], { tail: { count: 0, amount: "" }, pending: { count: 4 } }))} selectedQueue="" onSelectQueue={noAction} actions={inert} />);
    ...
    rerender(<Review result={ready(data([], { tail: null, pending: { count: 0 } }))} selectedQueue="" onSelectQueue={noAction} actions={inert} />);
    ...
    rerender(<Review result={ready(data([], { tail: null, pending: null }))} selectedQueue="" onSelectQueue={noAction} actions={inert} />);
  ...
  it("gives the empty state something focus can land on when a write empties the queue", () => {
    const { getByText } = render(<Review result={ready(data([]))} selectedQueue="" onSelectQueue={noAction} actions={inert} />);
...
describe("answering in a person's own words", () => {
  ...
  it("renders no form for a question nothing said in words settles", () => {
    ...
    // refusing.
    const { queryByRole } = render(<Review {...props} result={answerable([])} actions={inert} />);
  ...
  it("says what each slot needs back in the queue's own words", () => {
    const { getByText } = render(<Review {...props} result={answerable([slot])} actions={inert} />);
    expect(getByText(moments.wants_yes_no)).toBeInTheDocument();
  });

  it("shows the closed vocabulary an answer has to land in", () => {
    const { getByText } = render(<Review {...props} result={answerable([choiceSlot])} actions={inert} />);
  ...
  it("invites a person in the read's own sentence rather than one of its own", () => {
    const { getByLabelText } = render(<Review {...props} result={answerable([slot])} actions={inert} />);
  ...
  it("sends the question and the sentence, and nothing else", () => {
    ...
    const { getByLabelText, getByRole } = render(<Review {...props} result={answerable([slot])} actions={actions} />);
  ...
  it("sends nothing at all for an empty sentence", () => {
    ...
    const { getByRole } = render(<Review {...props} result={answerable([slot])} actions={actions} />);
    fireEvent.click(getByRole("button", { name: "Send this answer" }));
    expect(sent).toEqual([]);
  });
