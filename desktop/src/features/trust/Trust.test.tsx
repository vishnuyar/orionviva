...
import type { FeatureResult, TrustData, TrustSampleCapability } from "../../surface/types";
...
describe("Trust surface", () => {
  it("renders every FeatureResult boundary and both honest empty modes", () => {
    const { getByText, queryByText, rerender } = render(<Trust mode="live" result={{ state: "absent", reason: "not_read" }} />);
    expect(queryByText("Trust details unavailable")).not.toBeInTheDocument();
    rerender(<Trust mode="live" result={{ state: "unavailable", reason: "not_connected" }} />);
    expect(getByText("Trust and maintenance details are not connected to this private-vault read.")).toBeInTheDocument();
    rerender(<Trust mode="live" result={{ state: "failed", reason: "read_failed" }} />);
    ...
    rerender(<Trust mode="live" result={{ state: "partial", data: { notes: [note("partial")] }, issues: [{ code: "partial", message: "bounded" }] }} />);
    ...
    rerender(<Trust mode="live" result={{ state: "needs_input", data: { notes: [note("needs")] }, issues: [{ code: "needs", message: "bounded" }] }} />);
    ...
    rerender(<Trust mode="live" result={ready({ notes: [] })} />);
    ...
    rerender(<Trust mode="demo" result={ready({ notes: [], sample: { capabilities: [] } })} />);
  ...
  it("renders the exact preview boundary, all availability labels, and no settings actions", () => {
    ...
    const { container, getByText } = render(<Trust mode="demo" result={ready({ notes: [note("preview", "Preview boundary")], sample: { capabilities } })} />);
  ...
  it("shows only supplied live notes and hides malicious preview capabilities", () => {
    ...
    const { getByText, queryByText } = render(<Trust mode="live" result={ready({ notes: [note("live-note", "Backend supplied note", "Backend supplied detail")], sample: { capabilities: [malicious] } })} />);
  ...
  it("bounds missing and duplicate Trust note identities without merging by label or order", () => {
    ...
    const { getAllByText, getByText, queryByText, rerender } = render(<Trust mode="live" result={ready({ notes })} />);
    ...
    rerender(<Trust mode="live" result={ready({ notes: [...notes].reverse() })} />);
    expect(getAllByText("Same label")).toHaveLength(2);
  });
  ...
  it("renders missing note fields and bounded capability identity rows", () => {
    ...
    const { getAllByText, getByText, rerender } = render(<Trust mode="demo" result={ready({ notes: [note("missing-note", "", "")], sample: { capabilities } })} />);
    ...
    rerender(<Trust mode="demo" result={ready({ notes: [note("missing-note", "", "")], sample: { capabilities: [...capabilities].reverse() } })} />);
    expect(getAllByText("Capability identity unavailable")).toHaveLength(1);
    expect(getAllByText("Capability identity conflicted")).toHaveLength(1);
  });
  ...
  it("bounds inherited, future, and blank availability states without indexing the state map", () => {
    ...
    const { getAllByText, getByText } = render(<Trust mode="demo" result={ready({ notes: [], sample: { capabilities } })} />);
  ...
  it("keeps ready, missing, and conflicted identity keys disjoint across reorder", () => {
    ...
    try {
      ...
      const view = render(<Trust mode="demo" result={ready({ notes, sample: { capabilities } })} />);
      ...
      assertComplete();
      view.rerender(<Trust mode="demo" result={ready({ notes: [...notes].reverse(), sample: { capabilities: [...capabilities].reverse() } })} />);
      assertComplete();
  ...
  it("preserves long supplied identities and explanatory copy without truncation attributes", () => {
    ...
    const { container, getByText } = render(<Trust mode="live" result={ready({ notes: [note(long, long, long)] })} />);
