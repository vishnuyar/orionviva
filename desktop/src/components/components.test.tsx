...
describe("shared surface components", () => {
  ...
  it.each(["overview", "accounts", "activity", "documents", "conversation", "trust"])("bounds and recovers the %s destination by request-shaped reset key", (destination) => {
  ...
  it("renders a canonical figure as a dialog trigger", () => {
    const onOpen = vi.fn();
    const projected = accountEvidenceFigure({ id: "account", name: "Card", maskedNumber: "", kind: "card", measure: "owed", exactValue: "101", currency: "USD", display: "$101", grade: "verified", gradeLabel: "Verified", gradeDescription: "Verified", proofPresentation: { emphasis: "routine", reasons: [], qualifications: [] }, note: null, asOf: "today", coverage: null, provenance: null, evidenceLinks: [], state: "ready" });
  ...
  it("renders the complete evidence drawer in reviewed section order", () => {
    const account = { id: "account", name: "Card", maskedNumber: "", kind: "card", measure: "owed" as const, exactValue: "101", currency: "USD", display: "$101", grade: "verified" as const, gradeLabel: "Verified", gradeDescription: "Verified", proofPresentation: { emphasis: "routine" as const, reasons: [], qualifications: [] }, note: null, asOf: "today", coverage: "monthly", provenance: "Statement page 1", evidenceLinks: [{ targetDocumentId: "doc", label: "Card statement", relation: "same_account" as const, page: "page 1" }], state: "ready" as const, exactness: "rounded", recordIds: ["record-card-1"] };
    const snapshot: SurfaceSnapshot = { disclosure: sampleDisclosure, overview: { state: "ready", data: { picture: { coverage: "", readOn: "", figures: [], withheld: [], unplaced: [] }, accounts: [account] } }, documents: { state: "ready", data: { documents: [{ id: "doc", name: "card.pdf", state: "Verified", phaseLabel: "Verified", detail: "", source: "", pages: "1", provenance: "", evidenceLinks: [] }], readingSentence: "", captureQueue: [], processingJobs: [], outboundRecords: [] } }, activity: { state: "absent", reason: "" }, conversation: { state: "absent", reason: "" }, trust: { state: "absent", reason: "" } };
    const open = vi.fn();
  ...
  it("keeps the Evidence shell dismissible when its body throws and resets for another figure", () => {
    const first = { id: "first", name: "First account", maskedNumber: "", kind: "deposit", measure: "balance" as const, exactValue: "", currency: "USD", display: "$1", grade: "verified" as const, gradeLabel: "Verified", gradeDescription: "Verified", proofPresentation: { emphasis: "routine" as const, reasons: [], qualifications: [] }, note: null, asOf: "", coverage: null, provenance: null, evidenceLinks: [], state: "ready" as const };
    const second = { ...first, id: "second", name: "Second account", display: "$2" };
    const snapshot: SurfaceSnapshot = { disclosure: { title: "Private vault", subtitle: "Opened on this device", detail: "" }, overview: { state: "ready", data: { picture: { coverage: "", readOn: "", figures: [], withheld: [], unplaced: [] }, accounts: [first, second] } }, documents: { state: "unavailable", reason: "none" }, activity: { state: "absent", reason: "" }, conversation: { state: "absent", reason: "" }, trust: { state: "absent", reason: "" } };
    const dismiss = vi.fn();
  ...
  it("keeps missing and conflicted figure identities bounded", () => {
    const base = { id: "account", name: "Card", maskedNumber: "", kind: "card", measure: "balance" as const, exactValue: "1", currency: "", display: "$1", grade: "unavailable" as const, gradeLabel: "", gradeDescription: "", proofPresentation: { emphasis: "required" as const, reasons: ["test"], qualifications: ["A reviewed qualification."] }, note: null, asOf: "", coverage: null, provenance: null, evidenceLinks: [], state: "ready" as const };
    const makeSnapshot = (accounts: typeof base[]): SurfaceSnapshot => ({ disclosure: { title: "Private vault", subtitle: "Opened on this device", detail: "" }, overview: { state: "ready", data: { picture: { coverage: "", readOn: "", figures: [], withheld: [], unplaced: [] }, accounts } }, documents: { state: "unavailable", reason: "none" }, activity: { state: "absent", reason: "" }, conversation: { state: "absent", reason: "" }, trust: { state: "absent", reason: "" } });
    const badge = (grade: ReturnType<typeof accountEvidenceFigure>["grade"]) => <EvidenceBadge grade={grade.grade} label={grade.label} description={grade.description} />;
