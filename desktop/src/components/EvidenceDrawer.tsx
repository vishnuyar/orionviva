...
function measureLabel(figure: EvidenceFigureView) { if (figure.variant === "current-period") return "Known remainder"; switch (figure.measure) { case "balance": return "Balance"; case "owed": return "Amount owed"; case "spending": return "Spending"; case "income": return "Income"; case "net_worth": return "Net worth"; default: return "Measurement type was not supplied by this read."; } }
...
function EvidenceContents({ figure, snapshot, onOpenDocument, renderEvidenceBadge }: { figure: EvidenceFigureView; snapshot: SurfaceSnapshot; onOpenDocument: (link: EvidenceLink) => void; renderEvidenceBadge: (grade: GradePresentation) => ReactNode }) {
  return <div className="evidence-sections">
    ...
    <section><h3>Figure details</h3><dl className="evidence-detail-grid"><div><dt>Displayed figure</dt><dd>{figure.display || unavailableAmount}</dd></div><div><dt>Measure</dt><dd>{measureLabel(figure)}</dd></div><div><dt>Currency</dt><dd>{supplied(figure.currency, "Currency was not supplied by this read.")}</dd></div><div><dt>{figure.variant === "current-period" ? "Through" : "Measured"}</dt><dd>{supplied(figure.asOf, "Measurement date was not supplied by this read.")}</dd></div><div><dt>Evidence dates</dt><dd>{figure.evidenceDates.state === "supplied" ? figure.evidenceDates.value.join(", ") : "Evidence dates were not supplied by this read."}</dd></div><div><dt>Coverage</dt><dd>{figure.coverage.state === "supplied" ? figure.coverage.value.map((sentence) => <span className="evidence-coverage-line" key={sentence}>{sentence}</span>) : "Coverage was not supplied by this read."}</dd></div></dl></section>
    <section><h3>Records behind this figure</h3><p>These are record identities, not document links.</p>{figure.recordIds.state === "supplied" ? <ul className="record-identity-list">{figure.recordIds.value.map((recordId) => <li key={recordId}>{recordId}</li>)}</ul> : <p>Record identities were not supplied by this read.</p>}{figure.accountIds.state === "supplied" ? <><h4>Accounts behind this figure</h4><ul className="record-identity-list">{figure.accountIds.value.map((accountId) => <li key={accountId}>{accountId}</li>)}</ul></> : null}</section>
