...
const documents: DocumentsData = { documents: [{ id: "doc", name: "doc.pdf", state: "Verified", phaseLabel: "Verified", detail: "", source: "", pages: "1", provenance: "", evidenceLinks: [] }], captureQueue: [], processingJobs: [], outboundRecords: [] };
...
function snapshot(accounts: AccountView[] = [account], activityItems: ActivityView[] | null = [activity], recent: ActivityView[] = [activity]): SurfaceSnapshot {
  return { mode: "live", disclosure: { title: "Private vault", subtitle: "Opened", detail: "" }, overview: { state: "ready", data: { currentThrough: "", coverage: "", corpusCoverage: "", corpusSource: "", netWorth: figure, accounts, recent } }, documents: { state: "ready", data: documents }, review: { state: "absent", reason: "" }, activity: activityItems === null ? { state: "unavailable", reason: "not connected" } : { state: "ready", data: { items: activityItems } }, conversation: { state: "absent", reason: "" }, trust: { state: "absent", reason: "" } };
}
