...
import { loadPrivateSnapshot, privateActivityActions, privateConversationActions, privateDocumentActions, privateJobStream, privateReviewActions, privateSettingsActions, privateTransferActions, privateTrustActions, readEngineIdentity, readSurfaceRegistry, readUpdateLifecycle } from "./load-private-snapshot";
import type { ActivityActions, DocumentActions, EngineIdentity, FeatureResult, JobStream, ReviewActions, ConversationActions, SettingsActions, SurfaceRegistry, TrustActions, SurfaceSnapshot, UpdateLifecycleView, VaultTransferActions } from "./types";
...
export type SurfaceSource = { id: "bridge-client"; label: string; description: string; sample: boolean; frame: SampleFrame | null; load: () => Promise<SurfaceSnapshot>; reviewActions: ReviewActions; activityActions: ActivityActions; documentActions: DocumentActions | null; jobStream: JobStream | null; transferActions: VaultTransferActions | null; settingsActions: SettingsActions | null; conversationActions: ConversationActions | null; trustActions: TrustActions | null; describe: () => Promise<SourceDescription> };
