import type { DocumentsData, FeatureResult, OverviewData, ReviewData, SurfaceSnapshot } from "../types";

const unavailable = <T>(reason: string): FeatureResult<T> => ({ state: "unavailable", reason });
export function buildLiveSnapshot(overview: FeatureResult<OverviewData>, documents: FeatureResult<DocumentsData>, review: FeatureResult<ReviewData>): SurfaceSnapshot {
  return {
    mode: "live",
    ...
    trust: unavailable("Trust and maintenance details are not connected to this private-vault read."),
  };
}
