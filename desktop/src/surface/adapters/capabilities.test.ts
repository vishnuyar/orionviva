...
const declared = { overview: true, accounts: false, activity: false, documents: true, plans: true, review: false, viva: true, trust: false, settings: false, none: false };
...
describe("the registry the sidecar derived", () => {
  it("carries the sidecar's answer for every destination this shell has a screen for", () => {
    expect(adaptRegistry({ destinations: declared })).toEqual({
      served: { overview: true, accounts: false, activity: false, documents: true, plans: true, review: false, trust: false },
      undeclared: [],
    });
  });
  ...
  it("keeps a screen the registry never declared apart from one whose read is not connected", () => {
    ...
    expect(undeclared).toEqual(["accounts", "activity", "plans", "review", "trust"]);
