"""Compose Activity rows and controls from one projection."""
...
# Bounded Activity payload and control vocabularies.
DEFAULT_LIMIT = 50
VOCABULARY_LIMIT = 40
...
# Reviewed copy keys for movements whose nature is not plain spending.
...
def activity(projection, locale: str = "", limit: int = DEFAULT_LIMIT,
    ...
    ordered = sorted(movements,
                     ...
                     reverse=True)
    shown = ordered[:limit]
    if page is None and focus and all(movement.key != focus for movement in shown):
...
def _row(projection, movement, locale: str,
    ...
    return {
        ...
        # Carry stable identity and display name separately.
        "account_id": movement.account,
        "account_name": account_name,
        # Direction is the account-kind effect; value is unsigned.
        ...
        # Category provenance is present only for a complete classification.
        "classification": current["classification"],
        "tags": [_choice(tag) for tag in tags],
        # Cite the movement's recorded source coordinates.
        "evidence_links": _evidence_links(projection, movement),
        # Transfer controls derive from the closed relationship state.
        "transfer": transfer,
        "actions": actions,
    }
...
def _transfer_state(projection, movement, locale: str
    ...
    """Return the bounded transfer relationship and its supported actions."""
...
def _subcategory_choices(projection) -> list[dict[str, str]]:
    """Return accepted category/subcategory pairs with their parent identity."""
