...
from .prompts import STATEMENT_EXTRACTION_PROMPT
...
def read_statement(pdf_bytes: bytes, doc_id: str, spec: ModelSpec,
    ...
    if err is not None:
        # Try to surface the model's own classification even when the statement
        # parse failed (it may have correctly said "this is a pay stub").
        doc_type, conf = _peek_classification(result.text)
        return ReadResult(doc_type=doc_type, doc_type_confidence=conf,
                          facts=None, error=err)
    return ReadResult(doc_type=facts.doc_type,
                      doc_type_confidence=facts.doc_type_confidence, facts=facts)
