...
- [projection-decomposition-and-the-tool-registry.md](projection-decomposition-and-the-tool-registry.md) — the projection split into a core fold plus per-family view modules behind an unchanged facade, and the registry on top: honest tools, the `ToolResult` envelope with refusal first-class, vault-validated filters, evidence-bounded period reads, and the modality-neutral AnswerProgram compiler. Also the record of how a sentence is composed from shapes with typed holes.
- [answer-program-and-financial-query-engine.md](answer-program-and-financial-query-engine.md) — the delivery-ready replacement for the iterative answer loop: the model compiles a complete data-blind answer program, deterministic code validates and executes its bounded read graph, and one evidence gate binds and renders the result. Defines the contracts, Financial Query IR, resource policy, model admission, direct implementation phases, release gates, and deletion of the superseded path.
