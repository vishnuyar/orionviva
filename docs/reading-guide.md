...
- [implementation-roadmap.md](implementation-roadmap.md) — the checked record of what is built by capability, including quiet proof and bounded Activity correction reachability, followed by the six remaining dependency-ordered cycle families from obligations and quiet findings through explicitly confirmed action. Each future family still requires its own approved brief and crosses product, surface contract and installed interface as one vertical slice.
