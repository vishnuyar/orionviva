...
- [decisions/ADR-016-local-browser-interface.md](decisions/ADR-016-local-browser-interface.md) — optional same-computer browser access, retained native host, session ownership and browser-first acceptance.
...
- [account-identity-and-entity-resolution.md](account-identity-and-entity-resolution.md) — how one account is recognized across inconsistently-labelled statements: signals → deterministic match → ask only when ambiguous → settle the exact document and learn only safe signal rulings. The learning block reused later for merchants, employers and counterparties.
...
- [merchantcore-package.md](merchantcore-package.md) — the merchant enrichment package, a peer to vivacore that the product consumes: structural recognition, permanent merchant ids, reviewed exact aliases, MerchantRecord, Enricher, Catalog and commons. Installed matches sync back as events during import without a model call; enrichment remains a separate batched path for honest unknowns.
...
- [merchant-catalog-and-commons.md](merchant-catalog-and-commons.md) — categorize the merchant, not the transaction: structural candidates resolve through reviewed exact aliases onto permanent ids, while old local keys remain readable by grade. The raw descriptor stays encrypted; only a versioned, linted commercial catalog is shareable, and a network registry remains unbuilt.
...
- [encrypted-read-store-implementation-todo.md](encrypted-read-store-implementation-todo.md) — the approved, sliced implementation register for the implemented disposable encrypted desktop read model and the still-open containment, whole-system, packaging, Witness, and release gates. Checked items name evidence; the register does not claim release readiness.
- [historical-read-store-matrix.md](historical-read-store-matrix.md) — the value-time and source-order contract for historical SQL Activity and Overview reads, including the current-table shortcut that would give a false history.
...
- [projection-decomposition-and-the-tool-registry.md](projection-decomposition-and-the-tool-registry.md) — the projection split into a core fold plus per-family view modules behind an unchanged facade, and the registry on top: honest tools, the `ToolResult` envelope with refusal first-class, vault-validated filters, evidence-bounded period reads, and the modality-neutral semantic-request compiler over the retained AnswerProgram runtime. Also the record of how a sentence is composed from shapes with typed holes.
- [answer-program-and-financial-query-engine.md](answer-program-and-financial-query-engine.md) — the historical built-design record for the retained AnswerProgram runtime and the direct model-authorship experiment that failed its fresh Witness. Read it for the validator, query engine, evidence, binding, rendering, and why direct authorship was superseded.
- [semantic-request-and-deterministic-lowering.md](semantic-request-and-deterministic-lowering.md) — the current answer-boundary design: a model selects one reviewed financial meaning and typed parameters; one registry deterministically lowers it to the retained AnswerProgram runtime before any read. Defines the six primitive families, separate account inventory, structured capability gaps, capture/replay, and the exact 35-turn publication gate.
...
- [jobs-and-the-progress-channel.md](jobs-and-the-progress-channel.md) — the reviewed job registry and progress channel: sidecar-minted identities, live event delivery, honest step counts, cancellation checkpoints and bounded durable receipts. Interrupted work is restored as failed rather than silently resumed, and financial results still belong only in the event log.
