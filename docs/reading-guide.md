...
- [implementation-roadmap.md](implementation-roadmap.md) — the checked record of what is built by capability, followed by seven dependency-ordered cycle families for the planned path from quiet proof and correction reachability through explicitly confirmed action. Each future family still requires its own approved brief and crosses product, surface contract and installed interface as one vertical slice.
