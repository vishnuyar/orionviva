...
**Status:** Design spec — **both directions are BUILT.** Rulings in your own words (see [from-your-words-to-the-ledger.md](from-your-words-to-the-ledger.md)), and the read direction: a model plans over the tool registry and composes from tool results behind the citation gate (O1 built directly as the destination; `answer.py` remains the scripted test modality and the oracle). Stage C's machinery — the registry, the adapters, the planner, `viva.speak` — exists; its real-vault proving run is still owed. · **Created:** 2026-07-25 · **Origin:** Vishnu, using the debug surface to categorize: *"I am feeling a lot of deficiency and I do not want to create deterministic answers."*
