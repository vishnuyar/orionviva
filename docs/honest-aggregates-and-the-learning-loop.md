...
The learning loop is the other half. The rulings the system already writes — an account alias, a confirmed transfer, a human category, a merchant categorization — are one primitive implemented several times, and the question queue is one read-side projection gathering what still needs the person into one ranked list ([the-question-queue.md](the-question-queue.md)). Its ranking is by leverage and its scope is the most general unit that is still honest. A posted statement first gives each new movement a replaceable local default: a typed person starts as a transfer and anything else starts as `other`, without identifying the merchant or generalizing the answer. Activity keeps that treatment correctable as spending, a loan lent or a repayment. Questions remain for ambiguity no safe first answer covers. A human ruling is applied forever and retroactively; where neither a default nor a ruling settles a figure, the affected total stays provisional.
