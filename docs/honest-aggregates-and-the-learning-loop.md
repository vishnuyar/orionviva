# Honest Aggregates & the Learning Loop (Slice 6.5)

**Status:** ✅ **BUILT** — Move 1 (derived nature + honest aggregates), the reset guard, and Move 2 (the question queue, see [the-question-queue.md](the-question-queue.md)) · **Last updated:** 2026-07-25 · **Origin:** the first full real-vault run after Slice 6. The reported "spending" figure was materially inflated by internal money movement, and the largest category decomposed into two *opposite* natures under one label. **Blocks seeded:** movement **nature** (derived) · the **question queue** (the learning loop's front door).
...
- **Slice 3** records it as a `TransferLinked` overlay. Excluded from spending. ✅
- **Slice 5.5** records it as a *category* (`transfers`, `loan_payments`). **Still counted as spending.** ❌
...
2. **`transfer` — counterparty is an own account.** The movement names an account we hold (the Slice-3 token matcher already computes this), even if no link was formed — e.g. a card payment whose counterpart statement was never ingested. This is the rung that fixes the bulk of the error.
...
## Move 1 — derive nature (this build)
...
## Move 2 — the question queue (next, its own build)

The rulings we already write — `AccountAliasConfirmed`, `TransferLinked(by="human")`, `CategoryAssigned(by="human")`, `MerchantCategorized(by="human")` — are the same primitive implemented four times. Move 2 does **not** refactor those writers. It adds one read-side projection, `open_questions()`, that gathers everything the system is genuinely unsure about into a single list, each entry carrying:
...
The principle: **abstract the read side early (cheap, reversible); abstract the write side late (expensive, one-way).** A generic `Ruling` event is Move 3, and only once a fifth question type (Slice 8 or 11) proves the shape.
...
## As-built (Move 1 + the guard, 2026-07-25)
...
- **`provisional_spending()` and `excluded_from_spending()`** are surfaced in the web overview and `debug_vault` (what was excluded, by which rung, and how much rests on weak evidence).
