...
**A third reader reinvented it anyway, and the fix was to move the derivation rather than to write the rule down again (2026-08-16).** Within three days of `money_effect` being extracted, the streams module — the one the rhythm detector measures relationships with — was deciding direction as `in if amount > 0 else out`, consulting no account kind. Same shape once more: one fact, two places, the read listening to the weaker one. What was different is where it surfaced. Direction is half the key a rhythm hypothesis is grouped under, so one subscription paid from two kinds of account became two arrangements, each stating a total over half a relationship no sentence described — T1's *set a figure was taken over*, failing exactly as written — and the sentence a person would have been shown said the money came in from the merchant. The repair is this document's own principle applied literally: the derivation moved **down**, to the module that owns the words `in` and `out` and imports nothing from the ledger package, so the projection delegates to it rather than keeping a copy. One second opinion is still live and known as of 2026-08-16 — `implication_of` picks a counterparty's implication by the posted sign — and it is scheduled as its own cycle with a structural guard against a fifth; this sentence is deleted when that lands rather than being made true. A caller with no account kind raises rather than defaulting to the posted sign — the *route on the registry, not on the shape of the data* corollary, applied to a direction. Three losses for a rule that lived only as a function others were expected to call is what promoted it to a standing invariant (M2 in [design-invariants.md](design-invariants.md)).
