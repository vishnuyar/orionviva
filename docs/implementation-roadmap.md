...
replace the complete movement tag set, confirm or reject a suggested transfer,
or unlink a live one. Each action returns a typed outcome and the desktop
completes a full fresh surface read before replacing its financial picture; it
does not infer transfer candidates or patch a row optimistically.
