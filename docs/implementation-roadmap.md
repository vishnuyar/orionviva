...
## Slice 6 — Positions & investments  🟡 STAGE 1 DONE (snapshot)
**Block seeded:** Asset (valuation class) / Position (subtype + cost basis/lots).

_Stage 1 delivered (the holdings snapshot): the `brokerage_statement` divergent profile (kind `INVESTMENT`, identity `Σ market_value + cash = total`) + its prompt fragment; a `PositionObserved` measurement event (holdings recorded, never posted — Option A/M1); `post_brokerage` (internal-tally gate → hold on a misread, else open the investment account, observe cash, emit measurements); projection `positions()` / `account_value()` / `holdings_value()` / `unrealized_gain()` (the last a derived as-of view, never a ledger fact); surface + debug show the investment account's total value and its dated holdings. Tests: tally reconciles and records measurements; a misread holding is held; unrealized gain is derived (never an event); missing cost basis is absent not invented; a later statement revalues the same holding. **Stage 2 (cross-account cash flow — contributions tied via Slice 3, dividends/fees/realized gain) is next.**_
