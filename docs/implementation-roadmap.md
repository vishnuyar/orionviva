...
*Being asked, and answering.* A registry of read tools sits on the projection, each returning a graded, cited envelope rather than a bare number. A data-blind model receives the question, prior text context and a compact reviewed catalog, then returns one semantic family, typed parameters and requested claims. One registry owns that catalog, its schemas, deterministic builder lookup and admission digest. Code lowers the request to a complete `AnswerProgram`—answer shape, finite read/query DAG and pre-data selectors—and validates it before the first read. The first runtime scope is six primitive families; broad account inventory remains separately reviewed and unavailable to the model. The existing executor, Financial Query IR, evidence graph, binder and renderer remain the only grounded delivery path. A session carries prior turns as text context and re-fetches every figure per turn, so an answer is never composed from what an earlier answer said. The citation gate remains code rather than instruction: a financial figure with no record id behind it is refused, and one renderer writes every admitted value into typed holes. Nothing inspects a model-authored answer because the model authors neither answer prose nor executable structure ([semantic-request-and-deterministic-lowering.md](semantic-request-and-deterministic-lowering.md)). Every raw exchange, semantic request, lowered program, digest, validation result, execution trace and structured outcome is captured for replay. Runtime stays unavailable until the exact provider/model/modality/locale contract completes the seven exact questions five times each plus the reviewed paraphrases; deterministic tests alone cannot publish that profile, and the later private-vault Witness remains a separate gate.
