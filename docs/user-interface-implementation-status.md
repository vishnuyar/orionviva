...
| `has-file desktop/src/surface/adapters/review.ts` | Every live review row is read-only: no action, no outcome, no disposition. Answer, decline, proposal and confirmation are synthetic, and no live post-action refresh is connected. |
...
| `has-name product/viva/surface/models.py#ActionOutcome` | No shipped read path constructs one. The overview read now composes figures and a panel state; the contract generator reads this type's field names without ever building one; and every construction of it in the repository is in a test. **T1** and **VOICE-103** ride on a constructor no live action path calls, because no live action path exists. |
...
| `has-name product/viva/surface/capabilities.py#maturity` | Maturity is derived from reachability alone: a capability whose contract some operation serves reads stable, everything else reads preview. No operation in the table declares a served contract yet, so every entry in the registry reads preview and the field currently separates nothing from anything. Even once contracts are declared it stays uninformative for a capability with no destination, where it can only repeat "nothing serves this" — already true by construction. That is the accepted price of the field having one meaning, and no second field is added to fix it. |
...
| `has-file .github/workflows/release-desktop.yml` | Signing, notarization and updater upload run on tag across every target the release manifest declares, and the quality workflow builds the sidecar and the desktop bundle. What is outstanding is validation: no startup, shutdown or recovery check runs against a produced artifact in either workflow. |
