...
>
> This measurement predates the completed live desktop bridge. It is preserved
> because its anchored gaps explain the build, but many of its negative claims
> were closed. Use
> [the current implementation status](../user-interface-implementation-status.md).
...
| `bridge.open_demo_vault` | no | intercepted by `Sidecar.handle` as a branch before dispatch runs, beside the private open and in the same table; it carries no directory and no passphrase, because where the sample vault lives and what opens it are the engine's, and it mints that vault once where it lives rather than making a new one each launch | yes — it is the one affordance the sample vault is entered from, and the reply's frame words are what the shell draws around the place |
...
| `has-file product/viva/demo.py` | The sample vault is minted the first time a person opens it and then stays, so what they add to it or answer in it is there next time. Nothing removes it: there is no control that deletes the sample vault and no bound on how much a person can put into one, and the directory it lives in is the same one the product keeps its own settings in. Watched-folder capture exists nowhere at all — no host permission, no product entry point, no interface for it. |
