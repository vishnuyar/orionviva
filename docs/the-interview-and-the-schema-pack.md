...
- **The account is created by a confirmed Proposal, never by a tap.** The tier-2 yes returns a Proposal carrying a suggested name — from the merchant's canonical name and subcategory, as account resolution already does — and existing accounts are offered before a new one is minted. This is the audit finding's structural fix and it is not optional.
