...
- **Nothing the extraction model says auto-acts.** The agent that *does* things (the tool-using conversational Viva) is a separate context from the extraction worker, and its tools cannot move money or touch the network (agent-toolset forbidden list). Injection in a document cannot reach the tool surface.
